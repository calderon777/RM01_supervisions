...
These cut-offs are only heuristics. Evaluate VIFs alongside theory, sample size, and model purpose; high VIFs don’t automatically require dropping variables (O'Brien, 2007).
:::
...
chisq  <- N * Rsqres                        # Breusch–Pagan test statistic
...
**Decision rule:**
- If **p-value < 0.05** (or your chosen $\alpha$): reject $H_0$ → evidence against homoskedasticity (i.e., evidence of heteroskedasticity)
- If **p-value ≥ 0.05**: fail to reject $H_0$ → no strong evidence of heteroskedasticity
...
White's test extends Breusch–Pagan by including squares and cross-products of regressors in the auxiliary regression. This detects more general forms of heteroskedasticity (not just linear).
...
chisq2 <- N * Rsq2                          # Breusch–Pagan-type chi-square statistic
...
**Decision rule:**
- If **p-value < 0.05**: reject $H_0$ → evidence of heteroskedasticity (possibly nonlinear)
...
```{r}
# run Breusch–Pagan test and tidy the output
...
- If $F <$ lower critical value OR $F >$ upper critical value: reject $H_0$ → variances differ
- If $F$ lies between the two critical values: fail to reject $H_0$

**Decision rule (one-tailed test):**
- If $F >$ upper critical value (or p-value < 0.05): reject $H_0$ → variance in group 1 is larger
...
**Decision rule:**
- If **p-value < 0.05**: reject $H_0$ → variance increases with income
...
Positive correlation (upward diagonal pattern) → high growth tends to follow high growth
Negative correlation (downward diagonal pattern) → high growth followed by low growth
...
g: Real GDP growth rate (percentage change)
u: Unemployment rate
...
**In the lag-1 plot ($g$ vs $g_{t+1}$):**
- If points cluster along an **upward-sloping diagonal**, growth exhibits positive autocorrelation
- Most points in **upper-right** and **lower-left** quadrants → when growth is high today, it tends to be high next quarter
- This pattern suggests **persistence** or **momentum** in GDP growth

**In the lag-2 plot ($g$ vs $g_{t+2}$):**
- A weaker diagonal pattern suggests autocorrelation diminishes at longer lags
- If points become more randomly scattered, correlation decays over time
...
$\rho_k = \text{Corr}(g_t, g_{t-k}) = \dfrac{\text{Cov}(g_t, g_{t-k})}{\text{Var}(g_t)}$ <br>

where $k$ is the lag (1, 2, 3, ...).
...
Du <- diff(phill.ts[, "u"])                    # change in unemployment (∆u_t)

# FDL model: inf_t = β1 + β2 * ∆u_t + e_t
phill.dyn <- dynlm(inf ~ diff(u), data = phill.ts)                 # regress inflation on ∆unemployment
...
knitr::kable(dfr, caption = "Breusch–Godfrey test for the Phillips example.")  # nicely formatted summary
...
s2 <- coeftest(phill.dyn, vcov. = NeweyWest(phill.dyn))   # Newey–West (a common HAC estimator)
