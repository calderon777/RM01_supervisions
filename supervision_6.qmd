...
## Lab 19 — Qualitative Dependent Variables: Logit vs Probit on the Hedonic Model
...
### 🧠 From Continuous to Binary: Define High vs Low Price
...
::: callout-note
### ✨ Why logit & probit exist (the *aha!* moment)
...
### 🧩 Model Specification (same RHS as Supervision 5)
...
### ⚙️ Estimation (fit only) + 🪓 Backward Selection (BIC)

We now estimate the latent model using `glm()`:

$P(y = 1 \mid X) = \text{link}^{-1}(X\beta)$

- `logit` → inverse-logit (`plogis()`): returns a probability via *odds*
- `probit` → normal CDF (`pnorm()`): returns a probability via *z-score*

Logit uses the logistic CDF **(cumulative distribution function)**; probit uses the (scaled) normal CDF. They almost overlap, which is why results are so similar and logit ≈ 1.6 × probit in scale. Let's compare the CDFs:

```{r}
# Logistic vs Probit (variance-matched) — super short CDF plot
...
In the next chunk we **suppress printing** baseline models to avoid overwhelming output (too many variables). However, we run **backward stepwise (BIC)** and present a compact, readable table of that selected model. Coefficient signs and significance are typically very similar; logit coefficients are usually ~1.6× probit coefficients (scale difference).
...
best_model <- if (BIC(logit_bic) < BIC(probit_bic)) {
  message("✅ BIC selects the LOGIT model as best (more parsimonious).")
  logit_bic
} else {
  message("✅ BIC selects the PROBIT model as best (more parsimonious).")
  probit_bic
}
...
make_compact_table(main_logit, "Logit (BIC-selected): Key Terms — Robust SEs")
make_compact_table(main_probit, "Probit (BIC-selected): Key Terms — Robust SEs  📌 We will also use this downstream")
...
Logit/probit coefficients are **not** changes in probability — they change the *latent index*.

Marginal effects compute:

> “Holding everything else constant, how does  $P(y=1)$ change when $x$ increases by 1?”
...
### 📈 Interpreting Effects with Marginal Effects (AMEs)
...
Logit/probit coefficients are not probabilities—they shift the latent score, not $P(Y=1)$ directly. Marginal effects translate results into probability terms: “by how many percentage points does $P(Y=1)$ change when a variable increases by one unit, holding others constant?” Average Marginal Effects (AMEs) average this change over all observations, so the numbers can be read directly in percentage-point terms. Use AMEs for clear, substantive interpretation.
...
### 🔮 Predicted Probabilities & Agreement Between Models
...
### 🧪 Classification Diagnostics: ROC & AUC
...
ROC doesn’t evaluate the predicted **probability**, but the **ranking quality**.

> “Among all pairs of houses, how often does the model rank the expensive one higher?”

- AUC = 0.5 → random odds
- AUC = 1.0 → perfect ranking
:::

A ROC curve evaluates how well the model ranks positives above negatives across all possible thresholds. It does not measure overall “percent correct,” but the trade-off between true positive rate (sensitivity) and false positive rate as the cutoff moves. The AUC summarizes ranking ability from 0.5 (random) to 1.0 (perfect). Two models can have similar accuracy at one cutoff yet different AUCs, which is why ROC/AUC is informative.
...
autoplot_roc(roc_logit,  paste0("ROC – Logit (AUC = ", round(auc(roc_logit), 3), ")"))
autoplot_roc(roc_probit, paste0("ROC – Probit (AUC = ", round(auc(roc_probit), 3), ")"))
```

---

### ✂️ Choosing a Threshold & Confusion Matrices

Classification requires a threshold on predicted probabilities. The **Youden J** criterion often gives a better cut-off than the naïve 0.5.

Choosing a single cutoff is a separate decision from estimating probabilities. The default 0.5 is arbitrary and may be sub-optimal when the costs of false positives and false negatives differ. The Youden J statistic selects the threshold that maximizes sensitivity + specificity − 1, giving a balanced operating point. We will compare results at 0.5 and at the Youden-optimal threshold
...
Sensitivity = TP/(TP+FN) is a rate “how many actual true positives we catch from all true positives?” 

Specificity = TN/(TN+FP) asks “how many actual 0s did we avoid flagging?” 
...
### 📝 What to Report (suggested structure)
