...
We reuse the dataset prepared in Supervision 5 (e.g. `hedonic_ames.csv`). Make sure your **working directory** contains that file (or change the path accordingly).

```r
# If a package is missing on your machine, run install.packages("pkgname")
...
data_path <- "hedonic_ames.csv"

if (!file.exists(data_path)) {
  stop(paste0("File not found: ", data_path, "\nPlease place 'hedonic_ames.csv' in your working directory or set 'data_path' to the correct location."))
}

df <- read_csv(data_path)

# ---- Minimal cleaning / variable construction to match Supervision 5 RHS ----
# The following lines mirror the typical hedonic controls used previously.
# If your column names differ, adapt the code accordingly.
...
df <- df %>%
  mutate(
    ...
    # Ordinal/factor encodings (ordered where appropriate)
    ...
    # Bathrooms: total as full + 0.5 * half (common teaching convention)
...
### ⚙️ Estimation: Logit & Probit

We estimate both **logit** and **probit** via `glm()` by changing the link function. Coefficient signs and significance are typically very similar; logit coefficients are usually ~1.6× probit coefficients (scale difference).
