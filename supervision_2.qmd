...
# Lab 1 — The Simple Linear Regression Model

> 💡 **Note.** Students can adapt these R scripts to **UK rents annual inflation** and **UK house prices**.
...
Assumptions: (i) non-random \(x\); (ii) constant error variance (homoskedasticity); (iii) errors uncorrelated across observations; (iv) \(E[e_i\mid x_i]=0\).
...
# Lab 2 — Prediction with the Linear Regression Model
...
# Lab 5 — Hypothesis Test, p‑Value & Testing Linear Combinations
...
Right‑tail: \(p = 1 - F_t(t)\). Left‑tail: \(p = F_t(t)\). Two‑tail: \(p = 2\big(1-F_t(|t|)\big)\).
